...
=====================================
    ...
    Select your trajectory files. It is recommended to use a ``.tpr`` file for the topology and an ``.xtc`` file for the trajectory, though any MDAnalysis-supported format is compatible.
    ...
    The RSA class requires minimal user input. You must define the geometric thresholds and the trajectory range.
    ...
    .. code:: python
        ...
        # Stacking is considered if angle < ang_c OR angle > (180 - ang_c)
        ang_c = 30
    ...
    .. note::
       This version is optimized for polymer connectivity. To handle large systems (e.g., 100k+ atoms), it focuses on Polymer Residue IDs (``pol_resid``) rather than individual atom indices, significantly reducing memory overhead.
    ...
    You can generate PDB snapshots of specific interacting pairs to verify the stacking geometry in software like VMD, PyMOL, or using ``py3Dmol`` in a notebook.

    .. figure:: images/ring_stacking_snapshot_1.png
       :align: center
       :alt: Visualized stacking pair
    ...
    .. code:: python
        ...
        networks = rsa.find_several_rings_stacked(results)

        # Print the clusters found in the first frame
        print(f"Total isolated clusters: {len(networks[0])}")
        print(f"Largest cluster size: {len(max(networks[0], key=len))} polymers")
    ...
    .. code:: python
        ...
        print(networks[0][:2]) # Preview the first two clusters
    ...
    .. figure:: images/network_rings_stacked.png
       ...
       A representation of polymers color-coded by their stacking network components.
