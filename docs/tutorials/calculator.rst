...
=========================================================
Computing using enabled **ab-initio** codes 
=========================================================
...
.. note::
   ...
   - To globally enable the xtb_ executable, one can define the following
     two variables within your **~/.bashrc** file using these commands:

     **export PATH="$PATH:<PATH_WHERE_XTB_IS_STORED>/bin"**
     **export XTBHOME="<PATH_WHERE_XTB_IS_STORED>/bin"**
...
High-Throughput Calculations (HTPc) using PySoftK_
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
